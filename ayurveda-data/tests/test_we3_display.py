...
"""
    ...
    def test_dosha_line_labels_share_identity_icons_and_colors(self):
        ...
        self.assertIn("HStack(alignment: .center, spacing: 12)", scale)
        self.assertIn("titleFont: .caption.weight(.semibold)", scale)
        self.assertIn(".frame(width: 88, alignment: .leading)", scale)
        self.assertNotIn("titleFont: .headline", scale)
    ...
    def test_daily_ayurveda_summary_is_one_card_and_pin_has_large_hit_target(self):
        ...
        self.assertNotIn("distribution: profileDistribution ?? target", row)
        self.assertIn("private var foodEffectDistribution", row)
        self.assertIn("AyurvedaDisplayMath.percentages(", row)
        self.assertIn("if distribution != nil", row)
        self.assertIn("let markerTop = insetPoint(top, toward: centroid)", row)
        self.assertIn("let haloRect = CGRect(", row)
        self.assertIn("private var markerHaloColor: Color", row)
