...
final class CalendarViewModel: ObservableObject {
    ...
    init() {
        ...
        let persistedSelectionKey = selectedCalendarIDsKey
        $selectedCalendarIDs
            .sink { newValue in
                let array = Array(newValue)
                UserDefaults.standard.set(array, forKey: persistedSelectionKey)
            }
    ...
    private func isProfileCalendarTitle(_ title: String) -> Bool {
        title.hasSuffix(profileCalendarSuffix)
    }

    private func metadataEvent(in events: [EKEvent]) -> EKEvent? {
        events.first(where: { $0.title == profileMetadataEventTitle })
    }

    private func renameCalendarIfNeeded(_ calendar: EKCalendar, to title: String) {
        guard calendar.title != title else { return }
        calendar.title = title
        do {
            try eventStore.saveCalendar(calendar, commit: true)
            print("   🔄 Renamed calendar to: \(title)")
        } catch {
            print("   ❗️ Failed to rename calendar to '\(title)': \(error.localizedDescription)")
        }
    }
...
extension CalendarViewModel {
        ...
        if let listProfile = list.profile {
            ...
            if let sharedCalID = UserDefaults.standard.string(forKey: sharedShoppingListCalendarIDKey),
               let existingSharedCal = eventStore.calendar(withIdentifier: sharedCalID) {
                ...
                renameCalendarIfNeeded(existingSharedCal, to: sharedShoppingListCalendarTitle)
                return existingSharedCal
            }
            
            let calendarName = sharedShoppingListCalendarTitle
    ...
    func ensureSharedShoppingListCalendarExists() async {
        ...
        if let sharedCalID = UserDefaults.standard.string(forKey: sharedShoppingListCalendarIDKey),
           let calendar = eventStore.calendar(withIdentifier: sharedCalID) {
            renameCalendarIfNeeded(calendar, to: calendarName)
        ...
        if let foundByName = allCalendars.first(where: { $0.title == calendarName }) {
            ...
            renameCalendarIfNeeded(foundByName, to: calendarName)
    ...
    private func reconstructProfileData(from calendar: EKCalendar) async -> (UUID, ProfilePayload)? {
        ...
        guard let metadataEvent = metadataEvent(in: events) else {
    ...
    private func createOrUpdateMetadataEvent(
        ...
        let metadataEvent = metadataEvent(in: existingEvents) ?? EKEvent(eventStore: eventStore)
