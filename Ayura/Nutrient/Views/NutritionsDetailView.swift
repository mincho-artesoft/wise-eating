...
struct NutritionsDetailView: View {
    ...
    var body: some View {
        contentWithDeleteNoteAlert
    }

    private var contentWithAIOverlay: AnyView {
        AnyView(
            contentLayers
                .overlay {
    ...
    private var contentWithPrimaryAlerts: AnyView {
        AnyView(
            contentWithAIOverlay
                ...
                .alert("Delete Food", isPresented: $isShowingDeleteItemConfirmation, actions: {
                    ...
                    Text("Are you sure you want to delete '\(itemToDelete?.name ?? "this food")' from the meal? This will not affect your stored items.")
                })
        )
    }

    private var contentWithPresentationBehavior: AnyView {
        AnyView(
            contentWithPrimaryAlerts
                .presentationDetents([.large], selection: $selectedDetent)
                .presentationDragIndicator(.visible)
                .navigationBarTitleDisplayMode(.inline)
                .onChange(of: focusedGramsField) { _, newValue in
                    if newValue != nil { onInternalFieldFocused() }
                }
    ...
    private var contentWithPlanHandling: AnyView {
        AnyView(
            contentWithPresentationBehavior
                .confirmationDialog(
                    "Add Meals from Plan",
                    isPresented: .constant(planDaysToAdd != nil),
                    ...
                    message: {
                        Text("This will add meals for \(planDaysToAdd?.count ?? 0) day(s), starting from \(chosenDate.formatted(date: .abbreviated, time: .omitted)). How should meals for a specific time slot be added?")
                    }
                )
                .onReceive(aiManager.$jobs) { newJobs in
    ...
    private var contentWithDeleteNoteAlert: AnyView {
        AnyView(
            contentWithPlanHandling
    ...
    @ViewBuilder
    private var editorLayers: some View {
        ZStack {
...
//                                .padding(.horizontal, -10)
    ...
    private func handleAITap() {
        ...
        // 6.1 Рекламите са изключени глобално – продължаваме директно
        if !AdsConfiguration.shouldShowAds {
            print("🚫 Ads disabled: Starting daily nutrition detail generation directly.")
