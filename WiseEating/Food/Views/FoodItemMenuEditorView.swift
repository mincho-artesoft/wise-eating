...
struct FoodItemMenuEditorView: View {
    ...
    init(
        ...
        if let f = food {
            ...
            initialSelectedAllergens = allergens
    ...
    var body: some View {
        NavigationStack(path: $path) {
            ZStack {
                ...
                .onChange(of: focusedField) { oldValue, _ in
                    if case .ingredientGrams(let oldId) = oldValue {
                        formatIngredientText(for: oldId)
                    }
                    if oldValue == .minAge {
                ...
                if isSearchFieldFocused {
                    ...
                    // Изчисляваме кои храни вече са в менюто, за да ги скрием от резултатите
                    ...
                    FoodSearchPanelView(
                        ...
                        profile: profile,
                        // Търсим храни и рецепти (менюта обикновено не се слагат в други менюта, но може да го пуснеш ако искаш)
                        searchMode: .menus,
            ...
            // Navigation to PromptEditor
            .navigationDestination(for: NavigationTarget.self) { target in
                switch target {
                case .promptEditor:
                    PromptEditorView(promptType: .menu) { newPrompt in
                        path.removeLast()
                        if let newPrompt {
                ...
                case .editPrompt(let prompt):
                    PromptEditorView(promptType: .menu, promptToEdit: prompt) { editedPrompt in
                        if let editedPrompt, !selectedPromptIDs.contains(editedPrompt.id) {
            ...
            // Deletion confirmation for prompts
            .confirmationDialog(
                "Delete Prompt?",
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(spacing: 12) {
                ForEach(Array(selectedIng.keys.sorted(by: { $0.name < $1.name })), id: \.self) { item in
                    // Използваме новото генерично View
    ...
    @ViewBuilder
    private func ingredientRow(for item: FoodItem) -> some View {
        let gramsStringBinding = Binding<String>(
            get: { ingredientTextValues[item.id] ?? "" },
            set: { newText in
                ingredientTextValues[item.id] = newText
                if let displayValue = UnitConversion.parseDecimal(newText) {
                    let grams = isImperial ? UnitConversion.ozToG(displayValue) : displayValue
                    selectedIng[item] = grams
                } else if newText.isEmpty {
                    selectedIng[item] = 0.0
                }
            }
        )
        ...
        HStack {
            ...
            ConfigurableTextField(
                ...
                placeholderColor: effectManager.currentGlobalAccentColor.opacity(0.6),
                focused: $focusedField,
                fieldIdentifier: .ingredientGrams(item.id)
            )
    ...
    private func save() {
        Task { @MainActor in
            ...
            menu.diets = chosenDiets.isEmpty ? nil : chosenDiets
    ...
    private func loadFromDubFood() async {
        ...
        guard original.isMenu == true else {
            return
        }
        await MainActor.run {
            origRecipe    = original
    ...
    private func handleAITap() {
        ...
        if #available(iOS 26.0, *) {
            ...
            if let newJob = aiManager.startMenuGeneration(
                ...
                jobType: .menuGeneration
            ) {
                self.runningGenerationJobID = newJob.id
            } else {
                alertMsg = "Could not start AI menu generation job."
                showAlert = true
            ...
            alertMsg = "AI menu generation requires iOS 26 or newer."
            showAlert = true
        }
    }
    ...
    private func populateFromCompletedJob(jobID: UUID) async {
        guard let job = (aiManager.jobs.first { $0.id == jobID }),
            ...
            alertMsg = "Could not find completed job data for the menu."
            showAlert = true
        ...
        await MainActor.run {
            withAnimation(.easeInOut) {
                // Име (ако е върнато)
                if let newName = payload.name?.trimmingCharacters(in: .whitespacesAndNewlines),
                   !newName.isEmpty {
                    self.name = newName
                }
