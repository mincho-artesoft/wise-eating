...
| Integrated cold launch (INT-1, Debug simulator) | **1.461s** median vs `f46a107` 1.654s, N=10 same-session ABAB; paired median −0.195s. Warm Ayurveda check 0.226s→0.044s |
...
| FC-1e cold launch (Debug simulator) | candidate **1.433s** median vs `1a55e28` 1.425s, N=10 same-session ABAB; paired median +0.010s, smaller than both IQRs and not resolvable |
...
| MP-5 cold launch (Debug simulator) | candidate **1.401s** median vs `ba2c0d7` 1.403s, N=10 same-session ABAB; paired median −0.004s, smaller than both IQRs and not resolvable |
...
| INT-2 Phase 1 cold launch (Debug simulator) | candidate **1.437s** median vs `0015e83` 1.434s, N=10 same-session ABAB; paired median +0.002s, smaller than both IQRs and not resolvable |
...
| MP-1 planner telemetry | ⚠️ INTEGRATED / DEVICE EVIDENCE PENDING — `4b8307f` persists unchanged. Host/static integration gates pass; nine-run device matrix, G6 plan-identical diff, and G8 device launch are recorded in `DEFERRED-VALIDATION.md` and are not marked complete |
